...
def parse_args(input_args=None):
    ...
    if args.dataset_name is None and args.instance_data_dir is None and args.train_config_path is None:
        # raise ValueError("Specify either `--dataset_name` or `--instance_data_dir`")
        raise ValueError("Specify either `--dataset_name` or `--instance_data_dir` or `--train_config_path`")
    ...
    if args.with_prior_preservation:
        if args.reg_config_path is None:
            raise ValueError("You must specify a path to the regularization config file.")
        ...
        if args.reg_config_path is not None:
            warnings.warn("You need not use --reg_config_path without --with_prior_preservation.")
...
class DreamBoothDataset(Dataset):
    ...
    def __init__(
        ...
        if args.dataset_name is not None:
            ...
            # instance_images = [Image.open(path["file_name"]) for path in instance_dict_list]
            instance_image_captions = []
            instance_image_dir = "/".join(instance_config_path.split("/")[:-1])
            for instance_dict in instance_dict_list:
                img = Image.open(os.path.join(instance_image_dir, instance_dict["file_name"]))
                caption = instance_dict["text"]
                instance_image_captions.append((img, caption))
            # self.custom_instance_prompts = None

        self.instance_image_captions = []
        for img, caption in instance_image_captions:
            self.instance_image_captions.extend(itertools.repeat((img, caption), repeats))
        ...
        for image, caption in self.instance_image_captions:
        ...
        if self.class_config_path is not None:
            class_image_dir = "/".join(class_config_path.split("/")[:-1])
            class_dict_list = []
            with open(class_config_path, 'r') as f:
                for line in f:
                    try:
                        ...
                        break
            class_image_captions = []
            for class_dict in class_dict_list:
                img = Image.open(os.path.join(class_image_dir, class_dict["file_name"]))
                caption = class_dict["text"]
                class_image_captions.append((img, caption))
            
            self.class_original_sizes = []
            self.class_crop_top_lefts = []
            self.class_pixel_values = []
            self.class_caption = []
            for image, caption in class_image_captions:
                image = exif_transpose(image)
                if not image.mode == "RGB":
                    image = image.convert("RGB")
                self.class_original_sizes.append((image.height, image.width))
                image = train_resize(image)
                if args.random_flip and random.random() < 0.5:
                    # flip
                    image = train_flip(image)
                if args.center_crop:
                    ...
                    image = crop(image, y1, x1, h, w)
                crop_top_left = (y1, x1)
                self.class_crop_top_lefts.append(crop_top_left)
                image = train_transforms(image)
                self.class_pixel_values.append(image)
                self.class_caption.append(caption)
            self.num_class_images = len(class_image_captions)
    ...
    def __getitem__(self, index):
        ...
        caption = self.caption[index % self.num_instance_images]
        ...
        if self.class_config_path:
            # class_image = Image.open(self.class_images_path[index % self.num_class_images])
            class_image = self.class_pixel_values[index % self.num_class_images]
            class_original_size = self.class_original_sizes[index % self.num_class_images]
            class_crop_top_left = self.class_crop_top_lefts[index % self.num_class_images]
            class_caption = self.class_caption[index % self.num_class_images]
